...
main() {
  group("Account Bloc...", () {
    ...
    ''', () async {
      SharedPreferences.setMockInitialValues({AUTH_TOKEN_KEY: "some token"});
      await accountBloc.init();
      AccountBlocModel accountBlocModel = await accountBloc.stream.first;
      expect(accountBlocModel, HasLoginState(LoginState.loggedIn));
      expect(accountBlocModel, HasToken("some token"));
    });

    test(
        "CAN logoutAndDeleteData"
    ...
    test(
        "CAN set error"
        "When Service gives error"
        "SharedPreferences working", () async {
